...
Route::get('/', function () {
    return auth()->check() ? redirect()->route('portal.index') : view('welcome');
});

Route::middleware('guest')->group(function (): void {
    Route::get('/login', [PortalAuthController::class, 'create'])->name('login');
    Route::post('/login', [PortalAuthController::class, 'store'])->middleware('throttle:6,1')->name('portal.login.store');
});
